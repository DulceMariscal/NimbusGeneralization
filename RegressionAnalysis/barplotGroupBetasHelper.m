...
% 
    ...
    for i = 1:length(groupIDs)
        for j = 1 :num_coeff
            ...
            er = errorbar(i*(num_coeff+1) + j,coeff{i}.Estimate(j),coeff{i}.SE(j),coeff{i}.SE(j), 'Color','k','LineWidth',3); 
        end
    end

    set(gca,'XLim',[1*(num_coeff+1) (length(groupIDs)+1)*(num_coeff+1)],'XTick',[6,10,15,18,22],'XTickLabel',groupIDs);
    set(gca,'FontSize',18);
    ...
    legend(f([end,end-length(groupIDs)*2,end-length(groupIDs)*4, end-1]),legendLabels); %item plotted 1st, 10th and 20th
