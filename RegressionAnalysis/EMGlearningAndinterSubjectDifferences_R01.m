...
cd('/Users/dulcemariscal/Box/11_Research_Projects/LinearRegressionData_Betas')
%% Define data from params if necessary
% groupName='patients';
% subjIdx=[1:6,8:10,12:16]; %Excluding 7 and 11 which dont have short exp
% groupName='ControlApril2019';
% groupName='OldAdultsDataReduceAlt';
% groupName='OldAdultsData_WO_ADM_SAT_';
groupName='OldAdultsData';
subjIdx=2:16; %Excluding C01
% groupName='GYAAT';
% subjIdx=1:10; 
%% If EMGsummary file does not exist, run this:
%loadEMGParams_controls(groupName)
%loadEMGParams_youngadults(groupName)
%% Get all needed vars
% load(['../data/' groupName 'EMGsummary'])
% groupName='OldAdultsData';
load([groupName 'EMGsummary'])
% load ../data/bioData.mat
% load([groupName, 'bioData.mat'])
load('OldAdultsDatabioData.mat')
% load('GYAATbioData.mat')
% load('ControlApril2019bioData.mat')
if strcmp(groupName,'controls') || strcmp(groupName,'GYAAT') ||  strcmp(groupName,'OldAdultsData')
    vels=velsC;
else strcmp(groupName,'patients') 
    vels=velsS;
end
% write=true;
write=false;
...
tt=table(-median(eA(muscleIdx,subjIdx),2), -median(lA(muscleIdx,subjIdx),2), median(eAT(muscleIdx,subjIdx),2), median(eP(muscleIdx,subjIdx),2)-median(lA(muscleIdx,subjIdx),2),'VariableNames',{'eA','lA','eAT','eP_lA'});
...
ttSLA=table(-median(lA(muscleIdx,subjIdx),2), median(eAT(muscleIdx,subjIdx),2), -median(lS(muscleIdx,subjIdx),2), median(ePS(muscleIdx,subjIdx),2)-median(lS(muscleIdx,subjIdx),2),'VariableNames',{'lA','eAT','lS','ePS_lS'});

%%% SHORT WIH LONG EXPOSURE 
modelFitS2c=fitlm(ttSLA,'ePS_lS~lA+eAT-1','RobustOpts',rob)
learnS2c=modelFitS2c.Coefficients.Estimate;
learnS2cCI=modelFitS2c.coefCI;
r2S2c=uncenteredRsquared(modelFitS2c);
r2S2c=r2S2c.uncentered;
...
for i=1:size(eA,2)
    ttAll=table(-eA(muscleIdx,i), -lA(muscleIdx,i) , eAT(muscleIdx,i), eP(muscleIdx,i), eP(muscleIdx,i)-lA(muscleIdx,i),'VariableNames',{'eA','lA','eAT','eP','eP_lA'});  
...
%     r2All2c(i)=aux.uncentered;
    ...
    ttSLA_ind=table(-lA(muscleIdx,i), eAT(muscleIdx,i), -lS(muscleIdx,i), ePS(muscleIdx,i)-lS(muscleIdx,i),'VariableNames',{'lA','eAT','lS','ePS_lS'});
    ...
    modelFitAllS2c{i}=fitlm(ttSLA_ind,'ePS_lS~lA+eAT-1','RobustOpts',rob);
...
% BB=reshape(lB,336,16);
% BB=reshape(lB,312,16);
% BB=reshape(lB,264,16);
% BB=reshape(lB,120,10);
% BB=reshape(lB,96,16);
BB=1;
...
% if  strcmp(groupName,'GYAAT')
%    j=1;
% else
%     j=1:2;
% end
% for i=1:3 %Three models
%     for j=j %1:2 %Exposure length OR age comparison
...
%   
% end
%   scatter(rL(1),rL(2),150,'k','filled')
...
scatter(data3(subjIdx,1),data3(subjIdx,2),60,c1,'filled','MarkerFaceAlpha',.7)
%cc=colorbar;
xlabel(['\color[rgb]{' num2str(colorTransitions(1,1)) ',' num2str(colorTransitions(1,2)) ',' num2str(colorTransitions(1,3)) '} \beta_{env}'])
ylabel(['\color[rgb]{' num2str(colorTransitions(2,1)) ',' num2str(colorTransitions(2,2)) ',' num2str(colorTransitions(2,3)) '} \beta_{adapt}'])
ax=gca;
ax.XLabel.FontWeight='bold';
ax.XColor=colorTransitions(1,:);
ax.YColor=colorTransitions(2,:);
...
ax.YLabel.FontWeight='bold';
%set(cc,'Ticks',[0:.2:1],'TickLabels',num2str((max(age)-min(age))*[0:.2:1]' +min(age),2))
rL=modelFit2c.Coefficients.Estimate;
rS=modelFitS2c.Coefficients.Estimate;
hold on
if  strcmp(groupName,'GYAAT')
else
    scatter(rS(1),rS(2),150,c1,'filled')
